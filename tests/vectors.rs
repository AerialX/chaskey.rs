...
fn vectors() {
    ...
    for (i, vec) in VECTORS.iter().enumerate() {
        // sigh...
        let vec_bytes: Vec<u8> = vec.iter().cloned().map(u32::to_le_bytes)
            .map(|b| Vec::from_iter(b.iter().cloned()))
            .flatten().collect();

        msg[i] = i as u8; // [0], [0, 1], [0, 1, 2] and so on
        let mut chaskey = chaskey.clone();
        chaskey.process(&msg[..i]);
        chaskey.commit();
...
const VECTORS: &'static [TestVectors] = &VECTORS_8;

#[cfg(chaskey_rounds = "12")]
const VECTORS: &'static [TestVectors] = &VECTORS_12;

#[allow(unused)]
const VECTORS_8: [TestVectors; 64] = [
...
#[allow(unused)]
const VECTORS_12: [TestVectors; 64] = [
